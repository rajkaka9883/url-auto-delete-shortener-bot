        ...
        if total_results == 0:
            ...
            return await message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [[
                        InlineKeyboardButton("✧ ᴛᴀᴋᴇ ᴍᴏᴠɪᴇ ꜰʀᴏᴍ ʜᴇʀᴇ ✧", url=f"https://t.me/entertainment_baba"),
                        InlineKeyboardButton('⚜️ ᴏᴡɴᴇʀ ⚜️', url='https://telegram.me/PERSONAL_CHAT_ASSISTANT_BOT'),
...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn.insert(0, [
            InlineKeyboardButton("📍 ʜᴏᴡ ᴛᴏ ᴏᴘᴇɴ ʟɪɴᴋ 📍", url="")
        ])
    else:
        btn = []
        btn.insert(0, 
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://Full_Entertainment_Group")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://telegram.me/Full_Entertainment_Group")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
    ...
    # don't change anything without contacting me @LazyDeveloperr
        ...
        buttons = [[
            InlineKeyboardButton("🥹 ᴅᴏɴᴀᴛɪᴏɴ ɴᴏᴡ 🥹", url="https://t.me/PERSONAL_CHAT_ASSISTANT_BOT")
        ],[
        ...
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
            InputMediaPhoto("https://telegra.ph/file/1ff48f9e8a38854e21ee1.jpg")
        )
        await query.message.edit_text(
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
                    InlineKeyboardButton("☎️ Cᴏɴᴛᴀᴄᴛ", url="telegram.me/PERSONAL_CHAT_ASSISTANT_BOT")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
            m=await message.reply_text(f"<b>ᴀɪ ᴊɪꜱʜᴜ ʙᴀʙᴀ  2.0 ɪs ᴡᴏʀᴋɪɴɢ...</b>")
            search = search.lower()
            find = search.split(" ")
            search = ""
